...
/* ═══ BRANCHES ═══ */
async function branchLoad(){try{const d=await(await fetch('branch_api.php?action=dashboard')).json();if(!d.ok)throw new Error(d.msg);branchRenderDashboard(d);branchRenderList(d.branches);branchAnalyticsLoad()}catch(e){document.getElementById('branch-dashboard').innerHTML=`<div class="card" style="padding:2rem;color:#e74c3c;text-align:center">${e.message}</div>`}}
